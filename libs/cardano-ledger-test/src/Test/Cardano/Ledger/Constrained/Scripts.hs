...
allPlutusScripts proof = Map.fromList (map hash (concat [plutusByTag proof tag | tag <- [Spend .. Rewrd]]))
  where
...
spendPlutusScripts proof = Map.fromList (map hash (plutusByTag proof Spend))
  where
    hash (b, s) = (hashScript s, (b, s))

plutusByTag :: Proof era -> Tag -> [(IsValid, Script era)]
