...
freshP :: (Int, Subst era) -> Pat era t -> (Int, Subst era)
freshP (n, sub) (Pat _ as) = List.foldl' freshA (n, sub) as

freshA :: (Int, Subst era) -> Arg era t -> (Int, Subst era)
...
extendSums :: Subst era -> [Pred era] -> [Pred era]
extendSums _ [] = []
...
removeMetaSize ((MetaSize sz t@(Var v)) : more) ans = do
  n <- genFromSize sz
  let sub = substPred [SubItem v (Lit SizeR (SzExact n))]
...
mkDependGraph count prev (n : more) badchoices cs =
  ...
  where
    ...
    okE p@(SumsTo (Left _) t _ ns) =
      let lhsNames = List.foldl' varsOfSum Set.empty ns
       in if Set.isSubsetOf (varsOfTerm Set.empty t) prevNames
            && Set.disjoint prevNames lhsNames
            && Set.member n lhsNames
            then Left (Set.toList lhsNames, p)
            else Right p
    okE constraint =
