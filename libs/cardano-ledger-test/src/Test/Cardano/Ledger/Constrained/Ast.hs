...
data Pred era where
  ...
  Choose :: Eq t => Term era Size -> Term era [t] -> [(Target era t, [Pred era])] -> Pred era
  ForEach ::
    (FromList fs t, Eq t) =>
    Term era Size ->
    Term era fs ->
...
substFromPat pat = map (\(Name v) -> SubItem v (Var v)) (Set.toList (varsOfPat Set.empty pat))
